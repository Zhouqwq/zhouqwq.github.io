...
var trs = (function () {
  ...
  function MobiBackTop() {
    var dom =
      '<div id="mobi_back_top" class="m_back_top pchide"><a>回到顶部</a></div>';
  ...
  function SearchFn() {
    $("#searchBtn").on("click", function () {
      ...
      if (
        $("#iframe_header").length > 0 &&
        $(".header >.pchide:visible").length > 0
      ) {
        $("#iframe_header_p", parent.document).height(
          Math.ceil(($("#iframe_header").outerHeight() * winWidth) / 750)
        );
      }
    });
  }
...
$(function () {
  ...
  $("#godesktop").on("click", function () {
    ...
    $("meta[name='viewport']").attr(
      "content",
      "width=1280, target-densitydpi=device-dpi"
    );
    $("body").append(
      '<div id="showMobi" class="showMobi"><img src="/images/trs_mobiIcon.png" ignoreapd="true" />返回手机版</div>'
    );
  });
  // 返回手机版
