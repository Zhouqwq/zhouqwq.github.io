...
if (/Android (\d+\.\d+)/.test(ua)) {
    ...
    if (version > 2.3) {
        document.write(
            '<meta name="viewport" content="width=750, minimum-scale = ' +
                phoneScale +
                ", maximum-scale = " +
                phoneScale +
...
if (htmlEleO.getAttribute("data-notmobile")) {
    ...
    if (_phoneWidth == 1194 && _isIpad) {
        document.write(
            '<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no" id="meta_pad_fun">'
        );
